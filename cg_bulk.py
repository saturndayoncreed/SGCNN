...
class bulk:
    def unit_cell_expansion_bulk(self, lattices, matrix):
        num = len(lattices)
    ...
    def find_neighbor(self, lattices, expanded_lattices, elements):
        ...
        for i in range(lattice_num):
            ...
            for j in range(num):
                ...
                cond2 = np.linalg.norm(lattices[:, i] - expanded_lattices[:, j]) < (atom_features[elements[i]]['radius']
                                                                                    + atom_features[
    ...
    def bond_construction(self, elements, connectivity, distance):
    ...
    def poscar_to_graph(self, name):
        ...
        expanded_lattices = self.unit_cell_expansion_bulk(atoms, trans_matrix)
        ...
        bond_vectors, neighbor_indices = self.bond_construction(elements, connectivity, distances)
